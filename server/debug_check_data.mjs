...
const runDiagnosis = async () => {
    ...
    users.forEach(u => {
        // Handle cases where roles might be null or populated incorrectly
        const rolesList = u.roles ? u.roles.map(r => r ? `${r.name} (${r._id})` : 'null').join(', ') : 'None';
        console.log(`ID: ${u._id}, Name: ${u.name}, Role (String): "${u.role}", Roles (Array): ${rolesList}`);
    });
