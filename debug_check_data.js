...
import Role from './server/model/Role.js';
import WorkflowDefinition from './server/model/WorkflowDefinition.js';
...
const runDiagnosis = async () => {
    ...
    users.forEach(u => {
        console.log(`ID: ${u._id}, Name: ${u.name}, Role (String): "${u.role}", Roles (Array): ${u.roles.map(r => `${r.name} (${r._id})`).join(', ')}`);
    });
